...
var data_array = [
	...
	["LEVELED", "https://lu-sife.github.io/LEVELED/", "#C71266"],
...
//objects
const cube_instances = new Object;

var new_instance = "";
for (let i = 1; i < data_array.length + 1; i++) {
	new_instance = "cube";
	new_instance += i;
	cube_instances[new_instance] = new THREE.Mesh(geometry, material);
}
...
cube_instances.cube1.userData.URL = "https://lu-sife.github.io/Puzzle-Crawler/";
cube_instances.cube2.userData.URL = "https://github.com/LU-SIFE";
cube_instances.cube3.userData.URL = "https://lu-sife.github.io/LEVELED/";
cube_instances.cube1.scale.multiplyScalar(0.90);
cube_instances.cube2.scale.multiplyScalar(0.90);
cube_instances.cube3.scale.multiplyScalar(0.90);
outlineMesh.scale.multiplyScalar(1.05);
outlineMesh2.scale.multiplyScalar(1.05);
outlineMesh3.scale.multiplyScalar(1.05);

//adds object to the scene
scene.add(cube_instances.cube1);
scene.add(cube_instances.cube2);
scene.add(cube_instances.cube3);
...
cube_instances.cube2.position.x = 2;
outlineMesh3.position.x = -2;
cube_instances.cube3.position.x = -2;

outlineMesh2.position.z = -1;
cube_instances.cube2.position.z = -1;
outlineMesh3.position.z = -1;
cube_instances.cube3.position.z = -1;

camera.position.z = 5;

cube_instances.cube3.position.y = -0.4;
outlineMesh3.position.y = -0.4;

cube_instances.cube2.position.y = -0.5;
...
//checks which onject is being hovered
		if (intersects[0].object == cube_instances.cube1) {
			currently_moving = 1;
			return;
		}
		if (intersects[0].object == cube_instances.cube2) {
			currently_moving = 2;
		}
		if (intersects[0].object == cube_instances.cube3) {
			currently_moving = 3;
		}
		return;
	}
//reverts rotation amd material
		currently_moving = 0;
		cube_instances.cube1.material = material2;
		cube_instances.cube2.material = material2;
		cube_instances.cube3.material = material2;
...
function quaternion_revert(quat_select) {
	...
	if (quat_select == 1) {
		cube_instances.cube1.quaternion.rotateTowards(quaternion2, 0.1);
		outlineMesh.quaternion.rotateTowards(quaternion2, 0.1);
		return;
	}

	if (quat_select == 2) {
		cube_instances.cube2.quaternion.rotateTowards(quaternion2, 0.1);
		outlineMesh2.quaternion.rotateTowards(quaternion2, 0.1);
		return;
	}

	if (quat_select == 3) {
		cube_instances.cube3.quaternion.rotateTowards(quaternion2, 0.1);
...
function quaternion_rotate_towards(quat_select) {
	...
	if (quat_select == 1) {
		cube_instances.cube1.quaternion.rotateTowards(quaternion3, 0.1);
		outlineMesh.quaternion.rotateTowards(quaternion3, 0.1);
		return;
	}

	if (quat_select == 2) {
		cube_instances.cube2.quaternion.rotateTowards(quaternion3, 0.1);
		outlineMesh2.quaternion.rotateTowards(quaternion3, 0.1);
		return;
	}

	if (quat_select == 3) {
		cube_instances.cube3.quaternion.rotateTowards(quaternion3, 0.1);
...
function cube_bob() {
	...
	if (hover_offscreen == 1) {
		if (bobbing_counter == 0) {
			if (cube_instances.cube1.position.y < 0.20) {
				
				outlineMesh.position.y += 0.005;
				cube_instances.cube1.position.y += 0.005;
				return;
			}
			bobbing_counter = 1;
			return;
...
function cube2_bob() {
	...
	if (hover_offscreen2 == 1) {
			if (bobbing_counter2 == 0 && cube_instances.cube2.position.y < 0.20) {
				outlineMesh2.position.y += 0.005;
				cube_instances.cube2.position.y += 0.005;
				return;
			}
			
		bobbing_counter2 = 1;
		
		if (bobbing_counter2 == 1 && cube_instances.cube2.position.y > -0.20) {
...
function cube3_bob() {
	...
	if (hover_offscreen3 == 1) {
			if (bobbing_counter3 == 0 && cube_instances.cube3.position.y < 0.20) {
				outlineMesh3.position.y += 0.005;
				cube_instances.cube3.position.y += 0.005;
				return;
			}
			
		bobbing_counter3 = 1;
		
		if (bobbing_counter3 == 1 && cube_instances.cube3.position.y > -0.20) {
...
function calc_position() {
	...
	if (position_array[1] == 3) {
		cube_instances.cube3.position.x = 0;
		outlineMesh3.position.x = 0;
		cube_instances.cube3.position.z = 0;
	...
	if (position_array[1] == 2) {
		cube_instances.cube2.position.x = 0;
		outlineMesh2.position.x = 0;
		cube_instances.cube2.position.z = 0;
	...
	if (position_array[0] == 1) {
		cube_instances.cube1.position.x = -2;
		outlineMesh.position.x = -2;
		cube_instances.cube1.position.z = -1;
		outlineMesh.position.z = -1;
	}
	//cube1 in second/middle
	if (position_array[1] == 1) {
		cube_instances.cube1.position.x = 0;
		outlineMesh.position.x = 0;
		cube_instances.cube1.position.z = 0;
...
function link_change() {
	if (cube_instances.cube1.position.x == 0) {
		document.getElementById("puzzle_crawler").href = cube_instances.cube1.userData.URL;
		document.getElementById("puzzle_crawler").innerHTML = "Puzzle Crawler";
		return;
	}
	if (cube_instances.cube2.position.x == 0) {
		document.getElementById("puzzle_crawler").href = cube_instances.cube2.userData.URL;
		document.getElementById("puzzle_crawler").innerHTML = "My Github";
		return;
	}
	if (cube_instances.cube3.position.x == 0) {
		document.getElementById("puzzle_crawler").href = cube_instances.cube3.userData.URL;
